...
function SuggestedUser({user,setUser}) {
    ...
    const onFollowUser=async()=>{
        ...
        setUser({...user,
            followers:isFollowing ? user.followers.filter((followers)=>followers.uid !== authUser.uid) : [...user.followers,authUser]
        })
    }
  return (
    <Flex alignItems={"center"} justifyContent={"space-between"} w={"full"}>
        ...
        {authUser.uid !== user.uid && (
            <Button fontSize={13} bg={"transparent"} p={0} h={"max-content"} fontWeight={"medium"} color={"blue.400"} cursor={"pointer"} _hover={{
                color:"white"
